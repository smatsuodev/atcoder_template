...
#define _rses(i, s, e, step) \
  for (_n i = (_n)s; step < 0 ? i > (_n)e : i < (_n)e; i = i + (_n)(step))
#define _rsesi(i, s, e, step) \
  for (_n i = s; step < 0 ? i >= (_n)e : i <= (_n)e; i = i + (step))
...
template <class T>
T close_bound(const vector<T>& v, T& k) {
  T l = *(lower_bound(all(v), k) - 1);
  T u = *lower_bound(all(v), k);
  if (u == v[0]) ret u;
  if (abs(k - l) < abs(k - u)) {
    ret l;
  } else {
...
string join(const vector<T>& v, const string& d = " ") {
  ...
  rep(i, 1, v.size()) s += d + v[i];
  ret s;
}
template <class T>
void join_out(const vector<T>& v, const string& d = " ") {
  cout << v[0];
  rep(i, 1, v.size()) cout << d << v[i];
  cout << '\n';
}
template <class... T>
